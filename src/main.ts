...
export default class ChatGptViewProvider implements vscode.WebviewViewProvider {
  ...
  */
  constructor(private context: vscode.ExtensionContext) {
    ...
    vscode.commands.registerCommand("m31Mini.setOpenAIApiKey", async (apiKey: string) => {
      if (this.authStore) {
        ...
        console.error("[M31 Mini] Auth store not initialized");
      }
    });
    vscode.commands.registerCommand("m31Mini.getOpenAIApiKey", async () => {
      if (this.authStore) {
        const apiBaseUrl = this.api?.getApiUrl() ?? baseUrl ?? vscode.workspace.getConfiguration("chatgpt").get("gpt3.apiBaseUrl") as string;
  ...
  */
  ...
  async testApiKey(apiProvider: ApiProvider | undefined = undefined): Promise<{
    ...
    try {
      ...
      console.error('[M31 Mini] Main Process - Error getting models', error);
    ...
    webviewView.webview.onDidReceiveMessage(async (data: BaseBackendMessage) => {
      switch (data.type) {
        ...
        case BackendMessageType.cleargpt3:
          ...
          this.logEvent("[M31 Mini] NOT IMPLEMENTED - gpt3-cleared");
          break;
        ...
        case BackendMessageType.stopGenerating:
          ...
          if (stopGeneratingData?.conversationId) {
            ...
            console.warn("[M31 Mini] Main Process - No conversationId provided to stop generating");
          }
          break;
        ...
        case BackendMessageType.runAction:
          ...
          try {
            ...
            console.error("[M31 Mini] Main Process - Error running action: " + actionId);
            console.error(error);
        ...
        case BackendMessageType.stopAction:
          ...
          if (stopActionData?.actionId) {
            ...
            console.warn("[M31 Mini] Main Process - No actionName provided to stop action");
          }
          break;
        ...
        case BackendMessageType.openExternalUrl:
          ...
          break;
        default:
          console.warn('[M31 Mini] Main Process - Uncaught message type: "' + data.type + '"');
          break;
      }
    });
  }
  ...
  async setModel(newModel: Model): Promise<Model> {
    ...
    if (!this.api.isAzure && apiBaseURL.includes("openai.com")) {
      ...
      if (newModel.id !== updatedModelId) {
        console.info(`[M31 Mini] Updated deprecated model "${newModel.id}" to "${updatedModelId}".`);
        return {
          ...newModel,
          id: updatedModelId,
        };
  ...
  public clearConversation(conversationId?: string) {
    ...
    if (conversationId) {
      ...
      console.error("[M31 Mini] Main Process - No conversation to clear");
    }
  }
  ...
  public async exportToMarkdown(conversation: Conversation) {
    ...
    if (conversation) {
      ...
      console.error("[M31 Mini] Main Process - No conversation to export to markdown");
    }
  }
  ...
  public async sendApiRequest(prompt: string, options: ApiRequestOptions) {
    ...
    try {
      ...
      console.error("[M31 Mini] api-request-failed info:", JSON.stringify(error, null, 2));
      console.error("[M31 Mini] api-request-failed error obj:", error);
  ...
  listenForOpenRouterCallback(code_verifier: string,
    ...
    const server = createServer(async (req, res) => {
      ...
      if (url.searchParams.has('code')) {
        ...
        if (code) {
          if (!this.authStore) {
            console.error('[M31 Mini] [PKCE] No auth store found');
            return;
          }
          ...
          try {
            ...
            if (response.ok) {
              ...
              console.error(`[M31 Mini] Failed to exchange code for API key. Status: ${response.status}`);
            ...
            console.error(`[M31 Mini] Error exchanging code for API key: ${error}`);
          ...
          console.error(`[M31 Mini] No code provided. uri: ${url.toString()}`);
        }
      } else {
        console.error(`[M31 Mini] No code provided. uri: ${url.toString()}`);
      }
  ...
  async generateOpenRouterApiKey() {
    if (!this.authStore) {
      console.error("[M31 Mini] Main Process - AuthStore not initialized");
      return;
    }
    ...
    setTimeout(() => {
      if (server) {
        try {
          server.close();
          console.warn('[M31 Mini] OpenRouter callback server closed (5 min timeout)');
        } catch (error) { }
      }
    }, 300000);
  }
  ...
  private openSettings() {
    // Opens VS Code settings, filtered to just this extension's settings
    vscode.commands.executeCommand('workbench.action.openSettings', "@ext:m31-team.m31-mini chatgpt.");
  }

  private openPromptSettings() {
    // Opens VS Code settings, filtered to just the prompt prefix settings
    vscode.commands.executeCommand('workbench.action.openSettings', "@ext:m31-team.m31-mini promptPrefix");
  }
}
