...
import { setApiKeyStatus, setExtensionSettings, setModels, setReceivedExtensionSettings, setReceivedModels, setReceivedViewOptions, setViewOptions } from "./store/app";
...
import { ActionCompleteMessage, ActionErrorMessage, AddErrorMessage, AddMessageMessage, BaseFrontendMessage, FrontendMessageType, MessagesUpdatedMessage, ModelsUpdateMessage, SetConversationModelMessage, SettingsUpdateMessage, ShowInProgressMessage, StreamMessageMessage, UpdateApiKeyStatusMessage, UpdateMessageMessage, UpdateTokenCountMessage, ViewOptionsUpdateMessage } from "./types-messages";
...
export const useBackendMessageHandler = (backendMessenger: BackendMessenger) => {
  ...
  useEffect(() => {
    ...
    conversationList.forEach((conversation) => {
      if (models.some((model: Model) => model.id === conversation.model?.id)) {
        const updatedModel = models.find(
          (model: Model) => model.id === conversation.model?.id
        );
  ...
  return (event: { data: any }) => {
    if (debug) {
      console.info("[M31 Mini] Renderer - Received message from main process: ", event.data);
    }

    let message = event.data as ModelsUpdateMessage | SetConversationModelMessage | SettingsUpdateMessage | ViewOptionsUpdateMessage | UpdateApiKeyStatusMessage | MessagesUpdatedMessage | ShowInProgressMessage | UpdateMessageMessage | AddMessageMessage | StreamMessageMessage | AddErrorMessage | ActionCompleteMessage | ActionErrorMessage | UpdateTokenCountMessage;
    ...
    switch (message.type) {
      ...
      case FrontendMessageType.updateMessage: {
        ...
        if (updateMessageData?.chatMessage) {
          ...
          console.error("[M31 Mini] updateMessage event: No message provided");
        }
      ...
      case FrontendMessageType.addError: {
        ...
        const errorMessageText = "An error occurred. If this issue persists please clear your session token with `M31 Mini: Reset session` command and/or restart your Visual Studio Code. If you still experience issues, it may be due to an OpenAI outage. Take a look at https://status.openai.com to see if there's an OpenAI outage.";
      ...
      case FrontendMessageType.settingsUpdate: {
        ...
        if (!settingsUpdateData?.config) {
          console.warn("[M31 Mini] Renderer - No settings provided in settingsUpdate message");
          return;
        }
      ...
      case FrontendMessageType.viewOptionsUpdate: {
        ...
        if (!viewOptionsUpdateData?.viewOptions) {
          console.warn("[M31 Mini] Renderer - No view options provided in viewOptionsUpdate message");
          return;
        }
      ...
      case FrontendMessageType.actionComplete: {
        ...
        switch (actionCompleteData?.actionId) {
          case ActionNames.createConversationTitle:
            ...
            break;
          default:
            console.warn(`[M31 Mini] Renderer - Unhandled result from action: ${actionCompleteData?.actionId}`);
        }
        break;
      }
      ...
      default:
        console.error('[M31 Mini] Renderer - Uncaught message type: ', (message as BaseFrontendMessage)?.type);
    }
  };
};
